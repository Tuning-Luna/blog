...
/**
 * 内联 SVG 图标集 —— 从 design-system/components/Icon.tsx（React）按 DOM 契约
 * 移植为 Vue。品牌/强调图标为填充式，其余为 1.75px 描边 Lucide 风格。
 * 新增图标 = 往 FILLED/STROKE 映射表加 path；同一名字不得同时存在于两表。
 */
...
<template>
  <svg
    ...
    focusable="false"
  >
    <path :d="FILLED[props.name] || STROKE[props.name]" />
  </svg>
</template>
